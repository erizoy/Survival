        ...
        protected override void Update(GameTime gameTime)
        {
			...
			else
			{
				...
				if (p_choose)
				{
					...
					//cursor.Update(gameTime, u_indicator);
					if (m_mouse2.LeftButton == ButtonState.Pressed && perk.show_big) // перк здоров€к
					{
				...
				else
				{
					if (game_start)
					{
						...
						if (b_pistol) // ¬алера, пришло твое врем€
						{
							if (p_first_raised)
							{
								bullet.bullets.Clear();
							...
							if (!one_monster.isDead)
								foreach (bulletSprite one_bullet in bullet.bullets)
								{
									...
									if (one_monster.monsterRectangle.Intersects(bulletRectangle))
									{
										...
										{
											if (!one_monster.isDead)
												one_monster.rotationAngle = (float)Math.PI + one_bullet.angle;//угол падени€ трупа
